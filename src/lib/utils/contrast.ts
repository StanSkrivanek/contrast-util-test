...
export default function adjustFontColor(element: HTMLElement) {
	...
	if (ratio >= 4.5) {
		element.style.color = 'color-mix(in oklab, hsl(var(--_private-color)), hsl(0, 0%, 0%) 75%)';
	} else {
		element.style.color = 'color-mix(in oklab, hsl(var(--_private-color)) , hsl(0, 0%, 100%) 95%)';
	}
}
